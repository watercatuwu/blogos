...
<div class="flex flex-col gap-2">
    <div v-for="(post, index) in posts" :key="index" class="flex">
        <div class="flex flex-col p-4 bg-zinc-300 dark:bg-zinc-700 rounded-l-lg shadow-md w-2/3 space-y-2">
            <NuxtLink :to=post.path class="text-2xl hover:underline">{{ post.title }}</NuxtLink>
            ...
            <button v-if="href=='/'" @click="$emit('addPostWidget', post.path)" class="mt-auto flex items-center gap-1 rounded-full px-2 py-1 text-sm text-zinc-600 dark:text-zinc-400 bg-zinc-300 dark:bg-zinc-700 hover:bg-zinc-400 dark:hover:bg-zinc-600 transition-colors cursor-pointer">
